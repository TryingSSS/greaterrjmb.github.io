...
if($result->num_rows > 0) { 
 ...
 while($row = $result->fetch_array()) {
 	...
 	if($row[2] == 1) { 		
 		...
 		$paymentStatus = "<label class='label label-success'>Delivered</label>";
		}
	else if($row[2] == 3) { 		
 		$paymentStatus = "<label class='label label-warning'>Returned</label>";
 	} else { 		
 		$paymentStatus = "<label class='label label-danger'>None</label>";
 	} // /else
