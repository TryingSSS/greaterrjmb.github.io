...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			if(clientName == "") {
				$("#clientName").after('<p class="text-danger"> The Branch Name field is required </p>');
		...
		$("#orderDate").datepicker();
		$("#clientName").val(response.brand_name);
...
function printOrder(orderId = null) {
	if(orderId) {		
        ...
        mywindow.document.write('<html><head><title>Delivery Details</title>');        
...
function removeOrder(orderId = null) {
	...
	$.ajax({
			...
			success:function(response) {
				...
				var clientName = response.order[2];
				var total = response.order[8];
		...
		$("#removeOrderBtn").unbind('click').bind('click', function() {
			...
			$.ajax({
				...
				type: 'post',
				data: {orderId : orderId , clientName : clientName, total : total},
				dataType: 'json',
				success:function(response) {
	...
	if(orderId) {
		...
		$.ajax({
			...
			success:function(response) {	
				...
				$("#paymentStatus").val(response.delivery_status);
