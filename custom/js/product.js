...
$(document).ready(function() {
	...
	$("#addProductModalBtn").unbind('click').bind('click', function() {
		...
		$("#submitProductForm").unbind('submit').bind('submit', function() {
			...
			if(productImage && productName && quantity && rate && brandName && categoryName && supplierName && productStatus) {
	...
	if(productId) {
		...
		$.ajax({
		  ...
		  // 		allowedFileExtensions: ["jpg", "png", "gif", "JPG", "PNG", "GIF"]
				...
				$("#editqty").val(response.quantity_limit);
				
				$("#editqtyorder").val(response.quantity_order);
				...
				$("#updateSupplierDetailsForm").unbind('submit').bind('submit', function() {

					var quantity = $("#editqty").val();
					var qtyorder = $("#editqtyorder").val();
					var price = $("#editprice").val();
					var supplierName = $("#editsupplierName").val();

								
					if(quantity == "") {
						$("#editqty").after('<p class="text-danger">Quantity field is required</p>');
					...
					if(qtyorder == "") {
						$("#editqtyorder").after('<p class="text-danger">Quantity field is required</p>');
						$('#editqtyorder').closest('.form-group').addClass('has-error');
					}	else {
						// remov error text field
						$("#editqtyorder").find('.text-danger').remove();
						// success out for form 
						$("#editqtyorder").closest('.form-group').addClass('has-success');	  	
					}	// /else


					if(price == "") {
						$("#editprice").after('<p class="text-danger">Rate field is required</p>');
					...
					if(quantity && price && supplierName && qtyorder ) {
						// submit loading button
						$("#editSupplierDetailBtn").button('loading');
						...
						$.ajax({
							...
							success:function(response) {
								...
								if(response.success == true) {
									// submit loading button
									$("#editSupplierDetailBtn").button('reset');																		

									$("html, body, div.modal, div.modal-content, div.modal-body").animate({scrollTop: '0'}, 100);
																			
									// shows a successful message after operation
									$('#edit-supplierDetails-messages').html('<div class="alert alert-success">'+
				            '<button type="button" class="close" data-dismiss="alert">&times;</button>'+
				            '<strong><i class="glyphicon glyphicon-ok-sign"></i></strong> '+ response.messages +
				          '</div>');
